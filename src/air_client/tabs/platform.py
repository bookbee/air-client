...
def _send_turn(prefix: str, url: str, connection: Connection, body: dict[str, Any]) -> None:
    ...
    if not (exchange.ok and isinstance(exchange.response_json, dict)):
        return
...
def _send_row(
    ...
    right.markdown(
        ...
        f"{connection.channel} · {keyed}</span></div>",
        unsafe_allow_html=True,
    )
...
def render(customer: Connection, business: Connection) -> None:
    ...
    url = join_url(connection.base_url, route.path)
    _send_row(prefix, url, connection, errors, body)
